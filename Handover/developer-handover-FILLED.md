...
```bash
git clone https://github.com/JohnSite07/SPMS_Deployment
...
| --- | --- | --- |
| Production service | `https://spms-y5awp57u5q-uc.a.run.app` | Public HTTPS entry point, 100% of live traffic |
| Health check | `https://spms-y5awp57u5q-uc.a.run.app/health` | Used by the CD pipeline's smoke test; keep it dependency-free (no DB call) so it always answers, even during a cold start |
| Pre-release candidate | `https://candidate---spms-y5awp57u5q-uc.a.run.app` — Cloud Run prefixes the tag and three dashes onto the service's hostname (e.g. if the service URL is `https://spms-<hash>-uc.a.run.app`, the candidate is `https://candidate---spms-<hash>-uc.a.run.app`) | Every CD run deploys here first, with 0% production traffic, before the smoke test passes and traffic shifts. Useful for checking a revision before/without it becoming live — ask DevOps for the exact resolved URL of a given run, or read it from the CD run's logs (`Resolve candidate URL` step) |
...
gcloud sql instances describe spms-mysql --format="value(connectionName)"
# → spms-securevault:us-central1:spms-mysql
...
# 4. Connect
mysql -h 127.0.0.1 -P 3306 -u spms_app -p
```

`spms-mysql` is the Cloud SQL instance name (Terraform default: `spms-mysql`, see [terraform/modules/data/variables.tf](../../terraform/modules/data/variables.tf)). `spms_app` is the app login username, itself fetchable via `gcloud secrets versions access latest --secret=db-user`. The schema name is `securevault` by default (same file).
...
- **HTTPS endpoint**: `https://spms-y5awp57u5q-uc.a.run.app` is live for the full testing window; DevOps keeps it available.
...
## Filled-copy addendum (private — do not commit)

- GCP project ID: `spms-securevault` (run `gcloud config set project spms-securevault` after `gcloud auth login`)
- Live service: https://spms-y5awp57u5q-uc.a.run.app (health: `/health`)
- Cloud SQL: instance `spms-mysql`, connection name `spms-securevault:us-central1:spms-mysql`, database `securevault`, user `spms_app`
- Document bucket: `spms-securevault-spms-documents`
- Access prerequisite: your Google account must be in the DevOps team's `developer_emails` grant list — send your Google email + GitHub username to the DevOps team first.
