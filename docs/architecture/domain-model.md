...
- `Credential.encryptedPassword` and `SecureDocument.encryptedBlob` are ciphertext at rest (AES-256). The data model must not provide a path that persists their plaintext. **They are not stored the same way**: `encryptedPassword` is a MySQL column; `encryptedBlob` is an object in Cloud Storage, referenced from the `SECURE_DOCUMENTS` row by an opaque `object_key` — MySQL never holds document ciphertext. See [ADR 0017](../decisions/0017-secure-document-ciphertext-in-cloud-storage.md).
...
- `Credential.encryptedPassword` is no longer only a schema placeholder: [PRD 0019](../action_plan/0019-credential-vault-ui-and-encryption.md) built the client-side encryption that actually produces and consumes it (`client/src/services/vault-crypto.js`), plus the list/add/view/edit/delete UI on top of it (`Credentials.jsx`). The key-derivation design — direct KDF from the master password rather than a wrapped/rotatable key, the email-derived salt, and the resulting key lifecycle — is recorded in [ADR 0015](../decisions/0015-vault-key-derivation-from-master-password.md). `SecureDocument.encryptedBlob` is now implemented too: [PRD 0025](../action_plan/0025-secure-document-code-implementation.md) built client-side binary AES-256-GCM (`client/src/services/document-crypto.js`, extending `vault-crypto.js`'s primitives to raw bytes, reusing the same derived vault key) plus upload/list/download/delete UI (`Documents.jsx`) and the backend (`app/src/ports/documents.js`, `app/src/ports/blob-store.js`, `app/src/routes/documents.js`). Unlike `encryptedPassword`, the blob itself is never a MySQL column — it is stored in Cloud Storage, referenced by `SECURE_DOCUMENTS.object_key`; see [ADR 0017](../decisions/0017-secure-document-ciphertext-in-cloud-storage.md) for that storage-location decision and its two-system consistency model.
- `PasswordGenerator` is implemented as a pure client-side utility, not a persisted class: [`client/src/services/password-generator.js`](../../client/src/services/password-generator.js) (`generatePassword()`, `crypto.getRandomValues`-based) plus a separate graduated strength scorer, [`client/src/utils/password-strength.js`](../../client/src/utils/password-strength.js), wired into the Add/Edit Credential form (`Credentials.jsx`) per [PRD 0021](../action_plan/0021-password-generator.md). The strength score is a UX nudge for vault-item passwords and is deliberately distinct from — and does not gate on — the hard master-password rule (business rule 2) enforced elsewhere.
- `PasswordHealthReport` and `SecurityAlert`, and the `Vault`-to-`PasswordHealthReport` association above, are no longer only schema placeholders: [PRD 0022](../action_plan/0022-password-health-and-dashboard.md) built `+analyze()`'s real implementation — client-side only, since the server never holds the plaintext a weak/reuse check needs (`client/src/services/vault-health-analyzer.js`), persisted through `app/src/ports/password-health.js` and `app/src/routes/password-health.js` (`POST`/`GET /api/password-health`). The weak-vs-reused precedence rule, the score formula, the empty-vault edge case, and — most significantly — the resulting trust boundary (the server can verify a finding's `itemId` belongs to the caller, business rule 6, but not that the WEAK/REUSED/OK label itself is honest) are recorded in [ADR 0016](../decisions/0016-client-side-password-health-analysis.md). "Reused" is scoped to the current vault only, not business rule 9's stricter "or one used in the last 30 days" reading — a known gap, not a defect, also recorded there.
