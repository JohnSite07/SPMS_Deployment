...
These items were found reading the full PDF against the rest of this repo's docs. **All were Developer-team decisions** — items still open are listed here for awareness, not as resolved decisions.

1. ~~**Document blob storage conflict.**~~ **Resolved.** Part II's DDL stores document ciphertext as `SECURE_DOCUMENTS.encrypted_blob LONGBLOB` inside Cloud SQL, while Part III and this repo's own docs ([overview.md](overview.md)) describe encrypted document blobs going to a Cloud Storage bucket. [PRD 0024](../action_plan/0024-secure-document-storage-infra-and-handoff.md) settled the infra side in favour of Cloud Storage and fixed the storage contract (proxy-through-app upload, ADC auth, `DOCUMENTS_BUCKET`, ciphertext-in-GCS/metadata-in-MySQL, no auto-expiry retention) — see [deployment/document-storage.md](../deployment/document-storage.md). [PRD 0025](../action_plan/0025-secure-document-code-implementation.md) then shipped the code-level implementation: the reconciled `SECURE_DOCUMENTS` schema (metadata + `object_key`, no `encrypted_blob`/`file_iv`/`file_tag`), upload/list/download/delete routes, and the storage-boundary decision recorded as [ADR 0017](../decisions/0017-secure-document-ciphertext-in-cloud-storage.md).

2. ~~**Server-side vs. client-side encryption contradiction.**~~ **Resolved, for credentials and documents.** Parts I–III design a server-side `CryptoService` that reads the AES key from Secret Manager and encrypts/decrypts on the server. Part IV's "Features to be Tested" and several test cases (`TC-VAULT-03`, `TC-DOC-05`, `TC-SEC-04`) instead assert client-side / end-to-end encryption (e.g. "all data must be encrypted client-side before storage"; "reviewed... for any master password or [plaintext]... zero-knowledge architecture"). [PRD 0019](../action_plan/0019-credential-vault-ui-and-encryption.md) settled this for `Credential`/vault-item data: encryption is client-side, via `client/src/services/vault-crypto.js` deriving a key from the master password (never sent to or held by the server) — matching Part IV's assertion, not Parts I–III's server-side `CryptoService` design. See [ADR 0015](../decisions/0015-vault-key-derivation-from-master-password.md). [PRD 0025](../action_plan/0025-secure-document-code-implementation.md) extended the same boundary to `SecureDocument`: `client/src/services/document-crypto.js` encrypts file bytes client-side with the same derived vault key before upload; the server (and Cloud Storage) only ever handle opaque ciphertext. Note the server-side `AES_ENCRYPTION_KEY`/`services/crypto.js` path Parts I–III describe is real and still in use — `routes/two-factor.js` uses it to encrypt TOTP secrets at rest — it is simply not the path `routes/credentials.js` or `routes/documents.js` use; both `Credential.encryptedPassword` and `SecureDocument.encryptedBlob` are opaque client-produced ciphertext the server never touches cryptographically.
...
- [docs/deployment/README.md](../deployment/README.md) — the frontend pipeline note tied to item 3 above, now resolved.
