...
1. ~~**Document blob storage conflict.**~~ **Resolved, infra side.** Part II's DDL stores document ciphertext as `SECURE_DOCUMENTS.encrypted_blob LONGBLOB` inside Cloud SQL, while Part III and this repo's own docs ([overview.md](overview.md)) describe encrypted document blobs going to a Cloud Storage bucket. [PRD 0024](../action_plan/0024-secure-document-storage-infra-and-handoff.md) settles it in favour of Cloud Storage and fixes the storage contract (proxy-through-app upload, ADC auth, `DOCUMENTS_BUCKET`, ciphertext-in-GCS/metadata-in-MySQL, no auto-expiry retention) — see [deployment/document-storage.md](../deployment/document-storage.md). **Still open:** the code-level implementation (schema, upload/download routes, the storage-boundary ADR) is [PRD 0025](../action_plan/0025-secure-document-code-implementation.md)'s job, unbuilt as of this note.

2. ~~**Server-side vs. client-side encryption contradiction.**~~ **Resolved, for credentials.** Parts I–III design a server-side `CryptoService` that reads the AES key from Secret Manager and encrypts/decrypts on the server. Part IV's "Features to be Tested" and several test cases (`TC-VAULT-03`, `TC-DOC-05`, `TC-SEC-04`) instead assert client-side / end-to-end encryption (e.g. "all data must be encrypted client-side before storage"; "reviewed... for any master password or [plaintext]... zero-knowledge architecture"). [PRD 0019](../action_plan/0019-credential-vault-ui-and-encryption.md) settled this for `Credential`/vault-item data: encryption is client-side, via `client/src/services/vault-crypto.js` deriving a key from the master password (never sent to or held by the server) — matching Part IV's assertion, not Parts I–III's server-side `CryptoService` design. See [ADR 0015](../decisions/0015-vault-key-derivation-from-master-password.md). **Not yet resolved for `SecureDocument`** — document upload is unbuilt (see item 1), so which boundary it uses is still open. Note the server-side `AES_ENCRYPTION_KEY`/`services/crypto.js` path Parts I–III describe is real and still in use — `routes/two-factor.js` uses it to encrypt TOTP secrets at rest — it is simply not the path `routes/credentials.js` uses; `Credential.encryptedPassword` is opaque client-produced ciphertext the server never touches cryptographically.
