...
**Post-execution correction (2026-07-09, later):** the tfvars-only enable did not survive the pipeline — CD's next `terraform apply` (triggered by this PRD's own code push) ran without the local tfvars and reverted the instance to private within the hour. The operative control is now the **`ENABLE_PUBLIC_IP` GitHub Actions variable** (wired into CI plan + CD apply with a `|| 'false'` fallback); the pipeline restored the public IP under it and now keeps it converged. Committed default remains `false`, so the pipeline enforces private unless the variable says otherwise — the flip-back is `gh variable set ENABLE_PUBLIC_IP --body "false"` + one apply. ADR 0005 amended; runbook rewritten around the variable.
