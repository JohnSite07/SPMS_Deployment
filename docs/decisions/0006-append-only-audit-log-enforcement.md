...
- **Outstanding:** the database-level grant is not yet applied, and the first-created table has an `ON DELETE CASCADE` foreign key that would erase a user's trail on deletion — bypassing the grant entirely, since cascades run with the engine's privileges. The reconciled table (grant, `ON DELETE RESTRICT`, the association columns, millisecond precision) is specified in [architecture/audit-entries-schema.md](../architecture/audit-entries-schema.md). Flagged as a Developer-team schema concern in [PRD 0002](../action_plan/0002-network-and-data.md) and carried forward in [PRD 0008](../action_plan/0008-audit-log-and-vault-routes.md).
